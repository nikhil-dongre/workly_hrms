...
def get_work_hours(employee):
    now = timezone.now()
    today = now.date()
    atttendance_obj = Attendance.objects.filter(employee=employee,date=today).first()

    # all_attenlogs = AttendanceLog.objects.filter(attendance = atttendance_obj,
    #                       
    #                        punch_in=today,punch_out=today)
    all_attenlogs = Attendance.logs.all
    total_worked_hours=0
    for logs in all_attenlogs:
        if logs.punch_out:
            total_worked_hours+=(logs.punch_out-logs.punch_in)
